...
///
/// `prefer_anthropic_auth` 为真时优先用 `x-api-key` + `anthropic-version` 鉴权
/// （原生 Anthropic 端点不认 Bearer）；两种口径都会依次尝试。
...
pub async fn fetch_models_for_config(
    ...
    prefer_anthropic_auth: Option<bool>,
    ...
    model_fetch::fetch_models(
        ...
        models_url.as_deref(),
        user_agent,
        prefer_anthropic_auth.unwrap_or(false),
    )
    .await
}
